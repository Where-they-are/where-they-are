...
```text
apps/
├── web/                 # Next.js client portal; frontend-only
├── marketing/           # Astro SEO marketing site; static/frontend-only
├── server/              # Central NestJS backend; owns all backend logic
├── worker-whatsapp/     # whatsapp-web.js transport and AI intake adapter
├── worker-jobs/         # Background jobs and queue integration
└── site-origin/         # Public tenant/preview origin; proxies to server

packages/
├── config/              # Shared TypeScript/configuration conventions
├── contracts/           # Shared Zod schemas and API contracts
├── db/                  # Prisma 7/PostgreSQL; server-only database access
├── server-client/       # Typed HTTP client used by apps/workers
├── test-kit/            # Reusable unit, smoke, and Playwright verification
└── ui/                  # Shared UI package for frontend applications
```
